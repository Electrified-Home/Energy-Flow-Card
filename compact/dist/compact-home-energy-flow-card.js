...
`;var Jt=Object.defineProperty,V=(o,t,e,i)=>{for(var s=void 0,n=o.length-1,a;n>=0;n--)(a=o[n])&&(s=a(t,e,s)||s);return s&&Jt(t,e,s),s};const G=class G extends Dt{constructor(){super(...arguments),this.viewMode="compact",this.productionColor="#256028",this.batteryColor="#104b79",this.gridColor="#7a211b",this.returnColor="#7a6b1b",this.animation=new qt}static getStubConfig(){return{}}static getConfigForm(){return Ft()}disconnectedCallback(){super.disconnectedCallback(),this.animation.stop()}setConfig(t){const e=Nt(t);e.animation===void 0&&(e.animation=!1),this.config=e,this.isConnected&&(this.resetSubscriptions(),this.isAnimationEnabled()||this.animation.stop())}setupSubscriptions(){this.config&&(this.config.grid?.entity&&this.subscribe(this.config.grid.entity,()=>this.updateRenderData()),this.config.load?.entity&&this.subscribe(this.config.load.entity,()=>this.updateRenderData()),this.config.production?.entity&&this.subscribe(this.config.production.entity,()=>this.updateRenderData()),this.config.battery?.entity&&this.subscribe(this.config.battery.entity,()=>this.updateRenderData()),this.config.battery?.soc_entity&&this.subscribe(this.config.battery.soc_entity,()=>this.updateRenderData()))}updateRenderData(){if(!this.config||!this.hass)return;const t=this.hass.states[this.config.grid?.entity||""],e=this.hass.states[this.config.load?.entity||""],i=this.hass.states[this.config.production?.entity||""],s=this.hass.states[this.config.battery?.entity||""],n=parseFloat(t?.state??"0")||0,a=parseFloat(e?.state??"0")||0,c=parseFloat(i?.state??"0")||0;let r=parseFloat(s?.state??"0")||0;this.config.battery?.invert?.data&&(r=-r);const l=It({grid:n,production:c,load:a,battery:r});let h=null;if(this.config.battery?.soc_entity){const d=this.hass.states[this.config.battery.soc_entity];h=parseFloat(d?.state??"0")||0}this.viewMode=h!==null?"compact-battery":"compact",this.renderData={grid:n,load:a,production:c,battery:r,flows:l,batterySoc:h}}render(){if(!this.config||!this.renderData)return B`<ha-card class="compact-card"><div style="padding:16px;">Waiting for configuration...</div></ha-card>`;const{load:t,flows:e,battery:i,batterySoc:s}=this.renderData,n=this.isAnimationEnabled(),a=jt(t,e);return n&&this.animation.setLoadSpeed(t),B`
    ...
    `}renderLoadRow(t,e,i,s,n,a){return B`
    ...
    `}renderBatteryRow(t,e,i){const s=Vt(t,e);this.isAnimationEnabled()&&this.animation.setBatteryAnimation(t,s.direction);const n=s.gridIsImport?this.gridColor:this.returnColor,a=i!==null?i.toFixed(1):"--",c=t>0;return B`
    ...
    `}isAnimationEnabled(){return this.config?.animation!==!1}getIconFor(t){const e={grid:"mdi:transmission-tower",load:"mdi:home-lightning-bolt",production:"mdi:solar-power",battery:"mdi:battery"};return Wt(this.config,this.hass,t,e[t])}handleClick(t,e){this.hass&&zt(this.hass,this.fireEvent.bind(this),t,e)}fireEvent(t,e={}){if(t==="call-service"&&this.hass){this.hass.callService(e.domain,e.service,e.service_data||{},e.target);return}const i=new CustomEvent(t,{detail:e,bubbles:!0,composed:!0});this.dispatchEvent(i)}updated(t){super.updated(t),this.isAnimationEnabled()?!this.animation.isRunning()&&this.shadowRoot&&this.animation.start(this.shadowRoot):this.animation.isRunning()&&this.animation.stop(),t.has("renderData")&&this.renderData&&requestAnimationFrame(()=>{this.updateSegmentVisibility()})}updateSegmentVisibility(){if(!this.shadowRoot||!this.renderData)return;const{flows:t}=this.renderData,e=this.shadowRoot.querySelector(".compact-row:not(#battery-row) .bar-container");if(e){const i=this.shadowRoot.querySelector("#production-segment"),s=this.shadowRoot.querySelector("#battery-segment"),n=this.shadowRoot.querySelector("#grid-segment");if(i){const a=parseFloat(i.style.width)/100*e.clientWidth;D(i,a,t.productionToLoad>0)}if(s){const a=parseFloat(s.style.width)/100*e.clientWidth;D(s,a,t.batteryToLoad>0)}if(n){const a=parseFloat(n.style.width)/100*e.clientWidth;D(n,a,t.gridToLoad>0)}}if(this.viewMode==="compact-battery"){const i=this.shadowRoot.querySelector("#battery-row .bar-container");i&&i.querySelectorAll(".bar-segment").forEach(n=>{const a=parseFloat(n.style.width)/100*i.clientWidth,c=n.querySelector(".bar-segment-label"),r=!!(c?.textContent&&c.textContent.trim()!=="");D(n,a,r)})}}};G.styles=Gt;let A=G;V([lt({attribute:!1})],A.prototype,"config"),V([dt()],A.prototype,"viewMode"),V([dt()],A.prototype,"renderData");const ht="compact-home-energy-flow-card";customElements.get(ht)?console.info("[CompactHomeEnergyFlowCard] custom element already defined"):(customElements.define(ht,A),console.info("[CompactHomeEnergyFlowCard] defined custom element")),window.customCards=window.customCards||[],window.customCards.push({type:"compact-home-energy-flow-card",name:"Compact Home Energy Flow Card",description:"Compact bar visualization of home energy flows"})})();
