(function(){"use strict";function C(l){const t=Math.max(0,l.production),e=l.grid,i=l.battery,s=Math.max(0,l.load),n={productionToLoad:0,productionToBattery:0,productionToGrid:0,gridToLoad:0,gridToBattery:0,batteryToLoad:0};let o=t,r=s;if(o>0&&r>0&&(n.productionToLoad=Math.min(o,r),o-=n.productionToLoad,r-=n.productionToLoad),i<0&&o>0&&(n.productionToBattery=Math.min(o,Math.abs(i)),o-=n.productionToBattery),i>0&&r>0&&(n.batteryToLoad=Math.min(i,r),r-=n.batteryToLoad),r>0&&e>0&&(n.gridToLoad=Math.min(e,r),r-=n.gridToLoad),i<0&&e>10){const a=Math.abs(i)-n.productionToBattery;a>1&&(n.gridToBattery=Math.min(e-n.gridToLoad,a))}return e<-10&&(n.productionToGrid=Math.abs(e)),n}function T(l,t,e,i){const s=l[e];return s?s.name?s.name:s.entity&&t?.states[s.entity]&&t.states[s.entity].attributes.friendly_name||i:i}function E(l,t,e,i){const s=l[e];return s?s.icon?s.icon:s.entity&&t?.states[s.entity]&&t.states[s.entity].attributes.icon||i:i}class _{constructor(t,e,i,s,n,o,r,a,d=!1,h=!1,c,u,m){this.id=t,this._value=e,this.min=i,this.max=s,this.bidirectional=n,this.label=o,this.icon=r,this.units=a,this._invertView=d,this.showPlus=h,this.tapAction=c,this.entityId=u,this.fireEventCallback=m,this.element=null,this.radius=50,this.boxWidth=120,this.boxHeight=135,this.boxRadius=16,this.centerX=this.boxWidth/2,this.centerY=this.radius+25,this.offsetX=-this.centerX,this.offsetY=-this.centerY,this.needleState={target:0,current:0,ghost:0},this._lastAnimationTime=null,this._animationFrameId=null,this._updateNeedleAngle()}get value(){return this._value}set value(t){if(this._value!==t&&(this._value=t,this._updateNeedleAngle(),this.element)){const e=this.element.querySelector(`#value-${this.id}`);e&&(e.textContent=this._formatValueText()),this.updateDimming()}}get invertView(){return this._invertView}set invertView(t){if(this._invertView!==t&&(this._invertView=t,this._updateNeedleAngle(),this.element)){const e=this.element.querySelector(`#value-${this.id}`);e&&(e.textContent=this._formatValueText())}}get displayValue(){return this._invertView?-this._value:this._value}_formatValueText(){const t=this.displayValue,e=t.toFixed(0);return t<0?e+" ":t>0&&this.showPlus?"+"+e+" ":e}_updateNeedleAngle(){let t,e;const i=this.displayValue;if(this.bidirectional){const s=this.max-this.min;t=Math.min(Math.max((i-this.min)/s,0),1),e=180-t*180}else t=Math.min(Math.max(i/this.max,0),1),e=180-t*180;this.needleState.target=e}updateDimming(){if(!this.element)return;const t=this.element.querySelector(`#dimmer-${this.id}`);if(t){const e=Math.abs(this.value)<.5;t.setAttribute("opacity",e?"0.3":"0")}}startAnimation(){if(this._animationFrameId)return;const t=e=>{this._lastAnimationTime||(this._lastAnimationTime=e);const i=e-this._lastAnimationTime;if(this._lastAnimationTime=e,!this.element){this._animationFrameId=null;return}const s=this.radius-5,n=Math.min(i/150,1);this.needleState.current+=(this.needleState.target-this.needleState.current)*n;const o=Math.min(i/400,1);this.needleState.ghost+=(this.needleState.current-this.needleState.ghost)*o;const r=10;this.needleState.ghost<this.needleState.current-r?this.needleState.ghost=this.needleState.current-r:this.needleState.ghost>this.needleState.current+r&&(this.needleState.ghost=this.needleState.current+r);const a=this.element.querySelector(`#needle-${this.id}`);if(a){const h=this.needleState.current*Math.PI/180,c=this.centerX+s*Math.cos(h),u=this.centerY-s*Math.sin(h);a.setAttribute("x2",String(c)),a.setAttribute("y2",String(u))}const d=this.element.querySelector(`#ghost-needle-${this.id}`);if(d){const h=this.needleState.ghost*Math.PI/180,c=this.centerX+s*Math.cos(h),u=this.centerY-s*Math.sin(h);d.setAttribute("x2",String(c)),d.setAttribute("y2",String(u))}this._animationFrameId=requestAnimationFrame(t)};this._animationFrameId=requestAnimationFrame(t)}stopAnimation(){this._animationFrameId&&(cancelAnimationFrame(this._animationFrameId),this._animationFrameId=null,this._lastAnimationTime=null)}_handleTapAction(){if(!this.fireEventCallback)return;const t=this.tapAction||{action:"more-info"};switch(t.action||"more-info"){case"more-info":const i=t.entity||this.entityId;i&&this.fireEventCallback("hass-more-info",{entityId:i});break;case"navigate":t.path&&(history.pushState(null,"",t.path),this.fireEventCallback("location-changed",{replace:!1}));break;case"url":t.path&&window.open(t.path);break;case"toggle":this.entityId&&this.fireEventCallback("call-service",{domain:"homeassistant",service:"toggle",service_data:{entity_id:this.entityId}});break;case"call-service":if(t.service){const[s,n]=t.service.split(".");this.fireEventCallback("call-service",{domain:s,service:n,service_data:t.service_data||{},target:t.target})}break}}createElement(){const t=this.displayValue;let e,i;if(this.bidirectional){const v=this.max-this.min;e=Math.min(Math.max((t-this.min)/v,0),1),i=180-e*180}else e=Math.min(Math.max(t/this.max,0),1),i=180-e*180;this.needleState.target=i,this.needleState.current=i,this.needleState.ghost=i;const n=(this.bidirectional?[this.min,0,this.max]:[0,this.max/2,this.max]).map(v=>{const A=(180-(this.bidirectional?(v-this.min)/(this.max-this.min):v/this.max)*180)*Math.PI/180,P=this.radius,k=this.radius-8,D=this.centerX+P*Math.cos(A),X=this.centerY-P*Math.sin(A),B=this.centerX+k*Math.cos(A),O=this.centerY-k*Math.sin(A);return`<line x1="${D}" y1="${X}" x2="${B}" y2="${O}" stroke="rgb(160, 160, 160)" stroke-width="2" />`}).join(""),a=(180-(this.bidirectional?(0-this.min)/(this.max-this.min):0)*180)*Math.PI/180,d=this.centerX,h=this.centerY,c=this.centerX+this.radius*Math.cos(a),u=this.centerY-this.radius*Math.sin(a),m=`<line x1="${d}" y1="${h}" x2="${c}" y2="${u}" stroke="rgb(100, 100, 100)" stroke-width="2" />`,f=i*Math.PI/180,y=this.radius-5,g=this.centerX+y*Math.cos(f),p=this.centerY-y*Math.sin(f),w=this.centerY+5,x=this.centerY+this.radius*.5,z=this.centerY+this.radius*.7,q=`
    ...
    `,$=document.createElementNS("http://www.w3.org/2000/svg","svg");$.innerHTML=q;const b=$.firstElementChild;return this.element=b,(!this.tapAction||this.tapAction.action!=="none")&&(b.style.cursor="pointer",b.addEventListener("click",v=>{this._handleTapAction(),v.stopPropagation()}),b.addEventListener("mouseenter",()=>{b.style.filter="brightness(1.1)"}),b.addEventListener("mouseleave",()=>{b.style.filter=""})),b}}class F{constructor(t,e,i,s,n,o,r,a){this.group=t,this.flowId=e,this.speedMultiplier=r,this.dotsPerFlow=a,this.dots=[],this.dotStates=[],this.pathLength=0;const d=(i.x+s.x)/2,h=(i.y+s.y)/2;this.pathData=`M ${i.x},${i.y} Q ${d},${h} ${s.x},${s.y}`;const{opacity:c,strokeWidth:u,dotRadius:m}=this.calculateStyles(n),f=this.calculateVelocity(n);this.glowPath=document.createElementNS("http://www.w3.org/2000/svg","path"),this.glowPath.setAttribute("d",this.pathData),this.glowPath.setAttribute("class","flow-line"),this.glowPath.setAttribute("stroke",o),this.glowPath.setAttribute("stroke-opacity",String(c*.5)),this.glowPath.setAttribute("stroke-width",String(u*2)),this.glowPath.setAttribute("fill","none"),this.glowPath.setAttribute("stroke-linecap","round"),this.glowPath.setAttribute("style","transition: stroke-opacity 0.5s ease-out, stroke-width 0.5s ease-out;"),this.glowPath.id=`glow-${e}`,this.group.appendChild(this.glowPath),this.mainPath=document.createElementNS("http://www.w3.org/2000/svg","path"),this.mainPath.setAttribute("d",this.pathData),this.mainPath.setAttribute("class","flow-line"),this.mainPath.setAttribute("stroke",o),this.mainPath.setAttribute("stroke-opacity",String(c)),this.mainPath.setAttribute("stroke-width",String(u)),this.mainPath.setAttribute("fill","none"),this.mainPath.setAttribute("stroke-linecap","round"),this.mainPath.setAttribute("style","transition: stroke-opacity 0.5s ease-out, stroke-width 0.5s ease-out;"),this.mainPath.id=`path-${e}`,this.group.appendChild(this.mainPath),this.pathLength=this.mainPath.getTotalLength();for(let y=0;y<this.dotsPerFlow;y++){const g=document.createElementNS("http://www.w3.org/2000/svg","circle");g.setAttribute("class","flow-dot"),g.setAttribute("id",`dot-${e}-${y}`),g.setAttribute("r",String(m)),g.setAttribute("fill",o),g.setAttribute("opacity",String(c)),g.setAttribute("style","transition: opacity 0.5s ease-out, r 0.5s ease-out;"),this.group.appendChild(g),this.dots.push(g);const p=y/this.dotsPerFlow;this.dotStates.push({progress:p,velocity:f});const w=this.mainPath.getPointAtLength(p*this.pathLength);g.setAttribute("cx",String(w.x)),g.setAttribute("cy",String(w.y))}}calculateStyles(t){let e;t<=100?e=.25:t<=200?e=.25+(t-100)/100*.75:e=1;const i=2,s=23.76,n=1e4;let o;if(t<=100)o=i;else{const c=Math.min((t-100)/(n-100),1)*(s-i);o=i+c}const r=2.5,a=3,d=r*(o/i),h=Math.max(d,a);return{opacity:e,strokeWidth:o,dotRadius:h}}calculateVelocity(t){const e=40*(t/1e3)*this.speedMultiplier;return this.pathLength>0?e/this.pathLength:0}update(t,e){const{opacity:i,strokeWidth:s,dotRadius:n}=this.calculateStyles(t),o=this.calculateVelocity(t);this.glowPath.setAttribute("stroke",e),this.glowPath.setAttribute("stroke-opacity",String(i*.5)),this.glowPath.setAttribute("stroke-width",String(s*2)),this.mainPath.setAttribute("stroke",e),this.mainPath.setAttribute("stroke-opacity",String(i)),this.mainPath.setAttribute("stroke-width",String(s)),this.dots.forEach((r,a)=>{r.setAttribute("r",String(n)),r.setAttribute("opacity",String(i)),r.setAttribute("fill",e),this.dotStates[a].velocity=o})}animate(t){this.dotStates.forEach((e,i)=>{if(e.velocity>0){e.progress+=e.velocity*(t/1e3),e.progress>=1&&(e.progress=e.progress%1);try{if(this.pathLength>0){const s=this.mainPath.getPointAtLength(e.progress*this.pathLength);this.dots[i].setAttribute("cx",String(s.x)),this.dots[i].setAttribute("cy",String(s.y))}}catch{}}})}fadeOut(t){this.glowPath.setAttribute("stroke-opacity","0"),this.mainPath.setAttribute("stroke-opacity","0"),this.dots.forEach(e=>e.setAttribute("opacity","0")),setTimeout(t,500)}}class S{constructor(t,e){this.container=t,this.positions=e,this.flowLines=new Map,this.animationFrameId=null,this.lastAnimationTime=null,this.speedMultiplier=.8,this.dotsPerFlow=3,this.animate=()=>{const i=performance.now(),s=this.lastAnimationTime?i-this.lastAnimationTime:0;this.lastAnimationTime=i,this.flowLines.forEach(n=>{n.animate(s)}),this.animationFrameId=requestAnimationFrame(this.animate)}}updateFlows(t){const e=this.container.querySelector("#flow-layer");if(!e)return;const i=0,s=10;this.updateOrCreateFlow(e,"production-to-load",this.positions.production,this.positions.load,t.productionToLoad,"#4caf50",i),this.updateOrCreateFlow(e,"production-to-battery",this.positions.production,this.positions.battery,t.productionToBattery,"#4caf50",i),this.updateOrCreateFlow(e,"battery-to-load",this.positions.battery,this.positions.load,t.batteryToLoad,"#2196f3",s),this.updateOrCreateFlow(e,"grid-to-load",this.positions.grid,this.positions.load,t.gridToLoad,"#f44336",i),this.updateOrCreateFlow(e,"grid-to-battery",this.positions.grid,this.positions.battery,t.gridToBattery,"#f44336",i),this.updateOrCreateFlow(e,"production-to-grid",this.positions.production,this.positions.grid,t.productionToGrid,"#ffeb3b",i)}start(){this.animationFrameId||(this.lastAnimationTime=performance.now(),this.animate())}stop(){this.animationFrameId&&(cancelAnimationFrame(this.animationFrameId),this.animationFrameId=null,this.lastAnimationTime=null)}clear(){this.stop(),this.flowLines.clear();const t=this.container.querySelector("#flow-layer");t&&(t.innerHTML="")}updateOrCreateFlow(t,e,i,s,n,o,r){const a=this.flowLines.get(e);if(n<=r){a&&this.fadeOutFlow(t,e);return}a?a.update(n,o):this.drawFlow(t,e,i,s,n,o)}drawFlow(t,e,i,s,n,o){const r=document.createElementNS("http://www.w3.org/2000/svg","g");r.setAttribute("id",e),t.appendChild(r);const a=new F(r,e,i,s,n,o,this.speedMultiplier,this.dotsPerFlow);this.flowLines.set(e,a)}removeFlow(t,e){const i=t.querySelector(`#${e}`);i&&(i.remove(),this.flowLines.delete(e))}fadeOutFlow(t,e){const i=this.flowLines.get(e);i&&i.fadeOut(()=>{this.removeFlow(t,e)})}}class L{constructor(t,e,i,s,n,o){this.container=t,this.config=e,this.hass=i,this.getDisplayNameCallback=s,this.getIconCallback=n,this.fireEventCallback=o,this.meters=new Map,this.iconsExtracted=!1,this.iconExtractionTimeouts=new Set,this.iconCache=new Map,this.canvasWidth=500,this.canvasHeight=470;const r=5,a=3;this.meterPositions={production:{x:60+r,y:80+a},battery:{x:130+r,y:240+a},grid:{x:60+r,y:400+a},load:{x:360+r,y:240+a}}}setConfig(t){this.config=t}render(t){const{grid:e,load:i,production:s,battery:n,flows:o}=t,r=this.config.grid?.min??-5e3,a=this.config.grid?.max??5e3,d=this.config.load.max??5e3,h=this.config.production?.max??5e3,c=this.config.battery?.min??-5e3,u=this.config.battery?.max??5e3;if(!this.container.querySelector(".energy-flow-svg"))this.iconsExtracted=!1,this.initializeStructure(e,i,s,n,r,a,d,h,c,u),this.iconsExtracted||requestAnimationFrame(()=>{this.extractIconPaths()});else{const m=this.meters.get("production"),f=this.meters.get("battery"),y=this.meters.get("grid"),g=this.meters.get("load");if(m&&(m.value=s),f&&(f.invertView=this.config.battery?.invert?.view??!1,f.value=n),y&&(y.value=e),g&&(g.value=i),!this.flowRenderer){const p=this.container.querySelector(".energy-flow-svg");p&&(this.flowRenderer=new S(p,this.meterPositions),this.flowRenderer.start())}}this.flowRenderer&&this.flowRenderer.updateFlows(o)}stop(){this.flowRenderer&&this.flowRenderer.stop(),this.meters.forEach(t=>t.stopAnimation()),this.iconExtractionTimeouts.forEach(t=>clearTimeout(t)),this.iconExtractionTimeouts.clear()}clear(){this.stop(),this.flowRenderer&&this.flowRenderer.clear()}initializeStructure(t,e,i,s,n,o,r,a,d,h){const c=new _("production",i,0,a,!1,this.getDisplayNameCallback("production","Production"),this.getIconCallback("production","mdi:solar-power"),"WATTS",!1,!1,this.config.production?.tap,this.config.production?.entity,this.fireEventCallback),u=new _("battery",s,d,h,!0,this.getDisplayNameCallback("battery","Battery"),this.getIconCallback("battery","mdi:battery"),"WATTS",this.config.battery?.invert?.view,this.config.battery?.showPlus,this.config.battery?.tap,this.config.battery?.entity,this.fireEventCallback),m=new _("grid",t,n,o,!0,this.getDisplayNameCallback("grid","Grid"),this.getIconCallback("grid","mdi:transmission-tower"),"WATTS",!1,!1,this.config.grid?.tap,this.config.grid?.entity,this.fireEventCallback),f=new _("load",e,0,r,!1,this.getDisplayNameCallback("load","Load"),this.getIconCallback("load","mdi:home-lightning-bolt"),"WATTS",!1,!1,this.config.load.tap,this.config.load.entity,this.fireEventCallback);this.container.innerHTML=`
      <ha-card>
        <style>
          :host {
            display: block;
    ...
    `,requestAnimationFrame(()=>{const y=this.container.querySelector("#production-meter"),g=this.container.querySelector("#battery-meter"),p=this.container.querySelector("#grid-meter"),w=this.container.querySelector("#load-meter");y&&y.appendChild(c.createElement()),g&&g.appendChild(u.createElement()),p&&p.appendChild(m.createElement()),w&&w.appendChild(f.createElement()),this.meters.set("production",c),this.meters.set("battery",u),this.meters.set("grid",m),this.meters.set("load",f),c.startAnimation(),u.startAnimation(),m.startAnimation(),f.startAnimation(),c.updateDimming(),u.updateDimming(),m.updateDimming(),f.updateDimming();const x=this.container.querySelector(".energy-flow-svg");x&&!this.flowRenderer&&(this.flowRenderer=new S(x,this.meterPositions),this.flowRenderer.start())})}createMeterDefs(){return`
